...
if($_SESSION['tipo']=="administrador")
{
	...
	{ 
		...
		$sql = "DELETE FROM temas WHERE tema='$tema';"; // Como el campo id es num�rico, no necesita comillas simples en la cl�usula WHERE.
		...
		if (! $conexion){
			...
			if (! $resultado){
				...
				if(!$resultado){
				 	echo "ERROR: No pudo realizarse la operaci�n sobre la tabla $tabla.<br>\n";
				}
				else{
				 	$numero_registros_afectados=mysqli_affected_rows($conexion);
				 	echo "<h1>CORRECTO: Eliminación correcta de $numero_registros_afectados registros en la tabla $tabla.</h1><br>\n";
				}
			}
